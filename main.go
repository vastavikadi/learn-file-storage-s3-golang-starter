...
func main() {
	...
	configToUse, err := config.LoadDefaultConfig(r, config.WithRegion(s3Region))
	if err != nil {
		log.Fatal("Can't connect to aws s3 bucket")
	}

	cfg.s3Client = s3.NewFromConfig(configToUse)
